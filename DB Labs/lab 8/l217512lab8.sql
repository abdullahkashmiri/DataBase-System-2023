-- L21-7512 Abdullah Dar BSCS 4 G DB LAB 8
...
-- Question no 1
-- Create a UDF that takes cardNum and returns balance.


create Function Userbalancecard 
(@cardnum varchar(20)) 
returns float
...
Begin
declare @bal float
select @bal = balance from [Card] where cardNum = @cardnum;
return @bal
End 

select dbo.Userbalancecard  ('2343243253436') as Balance
...
create Function allUserDetails
(@uid int)
returns table
as 
return select  * from userdetail where userid = @uid

select * from allUserDetails (4)
...
-- Note: A user can have more than one cards
...
select cardNum , balance from allUserDetails (@uid)
End 

exec Userdetail_card 1
...
-- Question no 5
-- Write UDF that works the same as question#4.


create Function allUserDetailsF
(@uid int)
returns table
as 
return select cardNum , balance from allUserDetails (@uid)

select * from allUserDetailsF (1)
...
-- Question no 6
-- Create a procedure that takes the user’s id, and returns no. of cards of that user in an output parameter.
...
declare @result int
exec CardOwner'Ali', 1, @result  output
...
-- parameter. @status =1 if card number and PIN are valid and @status=0 otherwise.
...
-- PIN (Length 4) and prints “PIN is Updated” if PIN gets updated or “Error” otherwise.
...
-- Question no 9
-- Create a procedure WithDraw using Login Procedure (Question#6), the procedure will take Card
-- Number, PIN, an amount for the transaction. Now insert detail of the transaction in Transaction
-- Table if the transaction is successful with TransType =1(successful Transaction) and next transID
-- (calculate the max transID and increment transID, then insert in the table). If the transaction
-- fails insert TransType=4 (Failed Transaction). Update the balance against the card number
-- accordingly.
